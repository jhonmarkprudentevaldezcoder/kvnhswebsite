import React from "react";

export default function Updates() {
  return (
    <div className="container my-24 mx-auto md:px-6">
      <section className="mb-32 text-center">
        <h2 className="mb-12 pb-4 text-center text-3xl font-bold">
          Latest Updates
        </h2>

        <div className="grid gap-6 lg:grid-cols-3 xl:gap-x-12">
          <div className="mb-6 lg:mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">
                  OBECT / Oplan Balik-Eskwela Convergence Team
                </p>
              </div>
            </div>
          </div>

          <div className="mb-6 lg:mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">
                  SDO RIZAL Management Committee Meeting @ Ynares Event Center,
                  Antipolo City Montalban Sub Office 08/15/2023
                </p>
              </div>
            </div>
          </div>

          <div className="mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">
                  8/8/2023 HIRING PROCESS FOR TEACHER 2 APPLICANTS MONTALBAN SUB
                  OFFICE
                </p>
              </div>
            </div>
          </div>
        </div>
        <div className="grid gap-6 lg:grid-cols-3 xl:gap-x-12">
          <div className="mb-6 lg:mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">
                  The Official start of the School Year 2023-2024 for public
                  schools will be on August 28, 2023.
                </p>
              </div>
            </div>
          </div>

          <div className="mb-6 lg:mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">
                  During the G11 STEM ENTRANCE EXAM. AUGUST 2, 2023.
                </p>
              </div>
            </div>
          </div>

          <div className="mb-0">
            <div className="relative block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
              <div className="flex">
                <div
                  className="relative mx-4 -mt-4 overflow-hidden rounded-lg bg-cover bg-no-repeat shadow-lg dark:shadow-black/20"
                  data-te-ripple-init
                  data-te-ripple-color="light"
                >
                  <img
                    src="https://scontent.fmnl4-1.fna.fbcdn.net/v/t39.30808-6/347868448_998125101598580_1470542410123490152_n.jpg?_nc_cat=103&ccb=1-7&_nc_sid=a2f6c7&_nc_eui2=AeHXlUlgagDfokEDqP7K1AtLaU3vk56vAHppTe-Tnq8AeoYMOf8jrSxzi0q-tNZDb6gmQ9Uta9m0o5yQlrXfFWKf&_nc_ohc=JkIFCAhEwccAX9egsPN&_nc_ht=scontent.fmnl4-1.fna&oh=00_AfD29pqw1AVcyUCfuTbsctiBzVD9MzPGayqy02AjTvnEow&oe=651B53F4"
                    className="w-full"
                  />
                  <a href="#!">
                    <div className="absolute top-0 right-0 bottom-0 left-0 h-full w-full overflow-hidden bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100 bg-[hsla(0,0%,98.4%,.15)]"></div>
                  </a>
                </div>
              </div>
              <div className="p-6">
                <p className="mb-4 pb-2">8/7/2023 DAY 1 OF ENROLLMENT</p>
              </div>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
